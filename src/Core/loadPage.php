...
function loadPage(string $source, array $queries)
{
    if (extension_loaded('curl')) {
        ...
        foreach ($queries as $try) {
            ...
            $page = @file_get_contents(urlFromQuery($source, $try));
            if ($page) {
                return [$try, $page];
            }
        }
...
function urlFromQuery(string $source, string $query)
{
